  ...
  <v-container fluid class="pa-0 bg-grey-lighten-4">
        ...
        <v-row dense>
          ...
          <v-col cols="12" sm="6" md="3">
            <v-select
              ...
              "
              @update:model-value="queryByLocation"
            />
          </v-col>
    ...
    <v-container class="py-8">
      ...
      <v-row v-else>
        ...
        >
          <v-icon size="64" color="grey-lighten-2">mdi-car-search</v-icon>
          <p class="text-h6 text-grey mt-4">
            ບໍ່ພົບຂໍ້ມູນລົດແທັກຊີ່ທີ່ທ່ານຄົ້ນຫາ
          </p>
        </v-col>
      </v-row>
...
const queryByLocation = async () => {
  ...
  loading.value = true;
  try {
    const url = `https://service.tsheb.la/api/taxi/selectbyprovinceanddistrictid?provinceid=${selectedProvince.value}&districtid=${selectedDistrict.value}`;
    ...
    filteredData.value = processTaxiItems(finalData);
  } catch (e) {
  } finally {
    loading.value = false;
    console.groupEnd();
...
const openWhatsApp = (item) => {
  if (!item?.tel) {
    console.error("No telephone number found");
    return;
  }
  ...
  const whatsappUrl = `https://wa.me/856${cleanNumber}?text=${encodedMessage}`;
